...
def test_team_makes_a_draw():
  assert not ranking.team_makes_a_draw(1, match0)
  assert not ranking.team_makes_a_draw(3, match0)
  assert ranking.team_makes_a_draw(4, match1)
  assert ranking.team_makes_a_draw(2, match1)
  assert not ranking.team_makes_a_draw(3, match2)
  assert not ranking.team_makes_a_draw(2, match2)
  assert not ranking.team_makes_a_draw(4, match0)
...
def test_draw_match_count():
  for row in data.expected_sorted_ranking():
    assert ranking.draw_match_count(row['team_id'], data.matches()) == row['draw_match_count']
